...
function App() {
  ...
  const [formData, setFormData] = useState({ title: '', date: '', image: '', content: '', id: null });
  ...
  const saveEntry = () => {
    ...
    if (isEditing) {
      setEntries(entries.map(en => en.id === formData.id ? formData : en));
    } else {
      // FR009 Änderung: Mehrere Einträge pro Tag sind nun erlaubt (Prüfung entfernt)
      const newEntry = { 
        ...formData, 
        id: Date.now(), 
  ...
  const forceClose = () => {
    ...
    setFormData({ title: '', date: '', image: '', content: '', id: null });
  };
  ...
  const sortedEntries = [...entries].sort((a, b) => {
    // Erst nach Datum sortieren, bei gleichem Datum nach Erstellungs-ID (neueste oben)
  ...
  return (
    <div className="min-h-screen bg-[#F2F2F7] font-sans text-black pb-32">
      <header className="px-5 pt-14 pb-4 bg-[#F2F2F7]/80 backdrop-blur-md sticky top-0 z-40">
        <div className="flex justify-between items-end">
          <div>
            ...
            <h1 className="text-3xl font-extrabold tracking-tight">Diary</h1>
          </div>
          <div className="flex gap-4 items-center">
            ...
            {!isSelectMode && (
              <button 
                ...
                className="bg-[#007AFF] text-white w-9 h-9 rounded-full flex items-center justify-center shadow-md"
              >
        ...
        {sortedEntries.map((entry) => (
          <div key={entry.id} className="relative flex items-center gap-3">
            ...
            <div className="flex-1">
              {/* onDelete wird hier an die Card übergeben */}
              <EntryCard 
                entry={entry} 
                onClick={isSelectMode ? () => setSelectedIds(prev => prev.includes(entry.id) ? prev.filter(i => i !== entry.id) : [...prev, entry.id]) : setSelectedEntry} 
      ...
      {isSelectMode && (
        <div className="fixed bottom-8 left-0 right-0 flex justify-center z-50 px-5">
          <button 
            ...
            className="w-full max-w-xs bg-white/90 backdrop-blur text-red-500 py-4 rounded-2xl shadow-xl font-semibold border border-gray-200"
          >
      ...
      {/* Form Modal */}
      <Modal isOpen={isAddModalOpen} onClose={forceClose} onDone={saveEntry} title={isEditing ? "Edit Entry" : "New Entry"}>
        <div className="space-y-4">
          <div className="bg-gray-100 rounded-2xl p-1">
            <input type="text" placeholder="Title" className="w-full bg-transparent p-3 outline-none font-medium" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} />
          ...
          <div className="bg-gray-100 rounded-2xl p-6 border-2 border-dashed border-gray-300 flex flex-col items-center">
            {formData.image ? (
              <div className="relative w-full">
                <img src={formData.image} className="w-full h-48 object-cover rounded-xl shadow-inner" alt="Preview" />
          ...
          <textarea placeholder="Write your thoughts..." className="w-full bg-gray-100 p-4 rounded-2xl h-40 outline-none resize-none" value={formData.content} onChange={e => setFormData({...formData, content: e.target.value})} />
          <button onClick={saveEntry} className="w-full bg-[#007AFF] text-white py-4 rounded-2xl font-bold text-lg shadow-lg">
      ...
      {/* Detail View */}
      <Modal isOpen={!!selectedEntry} onClose={forceClose} onDone={forceClose} title={selectedEntry?.title}>
        {selectedEntry && (
          <div className="space-y-5 pb-10">
            <img src={selectedEntry.image} alt="" className="w-full aspect-[4/3] object-cover rounded-3xl shadow-sm" />
            <div className="flex justify-between items-center bg-gray-50 p-4 rounded-2xl border border-gray-100">
              <span className="text-gray-400 font-medium">{new Date(selectedEntry.date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</span>
              <div className="flex gap-2">
                <button onClick={() => { setFormData(selectedEntry); setIsEditing(true); setIsAddModalOpen(true); setSelectedEntry(null); }} className="p-2 bg-blue-50 text-[#007AFF] rounded-full">
                ...
                <button onClick={() => deleteSingle(selectedEntry.id)} className="p-2 bg-red-50 text-red-500 rounded-full">
